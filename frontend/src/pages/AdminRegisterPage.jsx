...
export default function AdminRegisterPage() {
  ...
  const [form, setForm] = useState({ name: '', email: '', phone: '', password: '', adminCode: '', bmsUrl: 'https://bms.libatech.net.lb', bmsUser: '', bmsPass: '' });
    ...
    <div className="min-h-screen bg-ms-bg flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <h1 className="font-semibold text-2xl text-ms-text tracking-tight mb-1">Admin registration</h1>
          <p className="text-ms-sub text-sm">Create an admin account and connect your BMS</p>
        </div>
        ...
        <form onSubmit={submit} className="ms-card p-6 space-y-4">
          ...
          <div>
            <label className="block text-xs text-ms-dim font-semibold mb-1.5">Phone</label>
            <input className="ms-input" type="tel" placeholder="+961 70 123 456" value={form.phone} onChange={set('phone')} required />
          </div>
          <div>
          ...
          <div>
            <label className="block text-xs text-ms-dim font-semibold mb-1.5">Admin Code</label>
            <input className="ms-input" type="password" placeholder="Provided by Aura Net" value={form.adminCode} onChange={set('adminCode')} required />
          </div>
